...
const openSearchIntegrationPipelineRole = new iam.Role(
    ...
    "OpenSearchIntegrationPipelineRole2",
    {
      ...
      inlinePolicies: {
        OSISInlineAccessPolicy: new iam.PolicyDocument({
          statements: [
            new iam.PolicyStatement({
              effect: iam.Effect.ALLOW,
              actions: ["aoss:*"], // or use the fine-grained ones again
              resources: ["*"], // or the scoped resources
            }),
            new iam.PolicyStatement({
              effect: iam.Effect.ALLOW,
              actions: [
              ...
              resources: [tableArn, `${tableArn}/export/*`, `${tableArn}/stream/*`],
            }),
            new iam.PolicyStatement({
              effect: iam.Effect.ALLOW,
              actions: [
